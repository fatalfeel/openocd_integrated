...
//by stone, prevent to call stack loop
		...
		char const *packet, int packet_size)
{
	...
	//if ((retval != ERROR_OK) && !gdb_report_data_abort) //by stone
	if ( (retval != ERROR_OK && !gdb_report_data_abort) || gdb_rmpacket_times >= 256 )
	{
		...
		char const *packet, int packet_size)
{
	...
	if (type == 0)	/* memory breakpoint */
		//bp_type = BKPT_SOFT; //by stone, only hardware breakpoint
		bp_type = BKPT_HARD;
